$(document).ready(function(){
    ...
    $("#workload\\.stuNumbers").change(function () {
        if (!isNumber($("#expperiod").val())) {
            alert("请输入实验学时!");
            $("#expperiod").focus();
            return;
        }
        var totalNumber = parseInt($("#workload\\.stuNumbers").val());
        var expperiod = parseInt($("#expperiod").val());
        var classNumbers;
        if (totalNumber < 35) {
            classNumbers=1;
        } else {
            classNumbers=Number(totalNumber / 35);
        }
        $("#workload\\.classNumbers").val(classNumbers.toFixed(2));
        $("#workload\\.labWorkload").val(fixed2(expperiod * classNumbers));
        $("#workload\\.preWorkload").val(fixed2(0.3 * $("#workload\\.labWorkload").val()));
        $("#workload\\.totalWorkload").val(fixed2(Number($("#workload\\.labWorkload").val()) + Number($("#workload\\.preWorkload").val())));
    });
});// JavaScript Document
...
function builderIndex(){
	$.each($("#items").children(),function(i,val){
		...
		$("#items").children("div").eq(i).children().eq(0).find("input[type=hidden]").eq(1).attr("name","items["+i+"].experimentWorkloadId");
...
function addExpperiod() {
    var period = $("#workload\\.cid option:selected").attr("period");
    $("#expperiod").val(period);
}
//数值保留两位小数
function fixed2(obj) {
