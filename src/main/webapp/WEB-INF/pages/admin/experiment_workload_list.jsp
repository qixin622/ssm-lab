...
<div class="admin">
    <div class="line">
        <div class="panel admin-panel">
            <div class="panel-head"><strong class="icon-reorder"> 实验工作量管理</strong></div>
            <div class="panel-body">
                <div class="padding-bottom border-bottom">
                    <form:form action="${ctx}/manage/experiment/workload/list" modelAttribute="workloadForm">
                        <ul class="search" style="padding-left:10px;">
                            <li>
                                <div class="button-group">
                                    <button type="button" class="button border-green"
                                            onclick="javascript:window.location.href='${ctx}/manage/experiment/workload/list';"><span
                                            class="icon-refresh"></span> 刷新数据
                                    </button>
                                    <button type="button" class="button border-main"
                                            onclick="javascript:window.location.href='${ctx}/manage/experiment/workload/to_add';"><span
                ...
                <table class="table table-bordered text-center">
                        ...
                        <c:forEach items="${workload.items}" var="item">
                            <tr>
                                <c:if test="${workload.id!=wid}">
                                    ...
                                    <td rowspan="${workload.items.size()}">${workload.course.expperiod}</td>
                                    ...
                                    <td rowspan="${workload.items.size()}">${workload.labWorkload}</td>
                                    <td rowspan="${workload.items.size()}">${workload.preWorkload}</td>
                                    <td rowspan="${workload.items.size()}">${workload.totalWorkload}</td>
                                    <td rowspan="${workload.items.size()}">${workload.deductPreWorkload}</td>
                                </c:if>
                                <td title="${item.teacherSn}">${item.teacher.userName}</td>
                                <td>${item.teacherWorkload}</td>
                                <c:if test="${workload.id!=wid}">
                                    <td rowspan="${workload.items.size()}">
                                        <a href="${ctx}/manage/experiment/workload/to_update?id=${workload.id}">编辑</a>
                                        <a href="${ctx}/manage/experiment/workload/remove?id=${workload.id}" onclick="return conf();">删除</a>
                                    </td>
                                </c:if>
                    ...
                    <tr>
                        <td colspan="7">工作量汇总：</td>
                        <td><fmt:formatNumber type="number" value="${totalLabWorkload}" pattern="#.00"/></td>
                        <td><fmt:formatNumber type="number" value="${totalPreWorkload}" pattern="#.00"/></td>
                        <td><fmt:formatNumber type="number" value="${totalWorkload}" pattern="#.00"/></td>
                        <td><fmt:formatNumber type="number" value="${totalDeductPreWorkload}" pattern="#.00"/></td>
                        <td colspan="3"></td>
                    </tr>
...
<script>
    ...
    $("#delete_all").click(function () {
        ...
        if (confirm("确认删除这些记录吗？")) {
            window.location.href = "${ctx}/manage/experiment/workload/remove?id=" + del_ids;
        }
    });
    $("#exportWorkload").click(function () {
        ...
        if (confirm("确定要导出记录吗？")) {
            window.location.href = "${ctx}/manage/experiment/workload/export?term="+term+"&workloadType="+workloadType;
        }
    });
</script>
</body>
