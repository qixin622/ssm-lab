            ...
            <div class="padding-large-left padding-large-right">
                <div class="panel margin-big-bottom">
                    <div class="panel-head">
                        <h1 class="text-main text-center icon-calendar"> 实践课程列表</h1>
                    </div>
                    <div class="panel-body">
                        <div class="line text-center margin-bottom">
                            <form method="post" action="${ctx}/pcourse/list" class="form-inline text-center">
                            ...
                            <div class="xm6 text-right">
                                <ul class="pagination pagination-group pagination-small border-main">
                                    ...
                                    <c:if test="${pageInfo.hasPreviousPage}">
                                        <li><a href="${ctx}/pcourse/list">首页</a></li>
                                        <c:if test="${type!=null and keywords!=null}">
                                            <li><a href="${ctx}/pcourse/list?pn=${pageInfo.pageNum-1}&type=${type}&keywords=${keywords}"><i
                                                    class="icon-angle-double-left"></i></a></li>
                                        </c:if>
                                        <c:if test="${type==null}">
                                            <li><a href="${ctx}/pcourse/list?pn=${pageInfo.pageNum-1}"><i
                                                    class="icon-angle-double-left"></i></a></li>
                                        </c:if>
                                    </c:if>
                                    <c:forEach items="${pageInfo.navigatepageNums}" var="num">
                                        ...
                                        <c:if test="${pageInfo.pageNum != num}">
                                            <c:if test="${type!=null and keywords!=null}">
                                                <li><a href="${ctx}/pcourse/list?pn=${num}&type=${type}&keywords=${keywords}">${num}</a></li>
                                            </c:if>
                                            <c:if test="${type==null}">
                                                <li><a href="${ctx}/pcourse/list?pn=${num}">${num}</a></li>
                                            </c:if>
                                        </c:if>
                                    </c:forEach>
